...
const AwsAuthProvider = ({ children }) => {
  ...
  const signUpCognitoUser = async ({ email, password, name }) => {
    ...
    try {
      ...
      dispatch({
        type: SHOW_MESSAGE,
        payload: 'A code has been sent to your registered email address, Enter the code to complete the signup process!',
      });
    } catch (error) {
      setAwsCognitoData({
        user: null,
  ...
  const confirmCognitoUserSignup = async (username, code) => {
    ...
    try {
      ...
      dispatch({
        type: SHOW_MESSAGE,
        payload: 'Congratulations, Signup process is complete, You can now Sign in by entering correct credentials!',
      });
    } catch (error) {
      setAwsCognitoData({
        user: null,
