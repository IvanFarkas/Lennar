...
const renderMenuItemChildren = (item) => {
  ...
  if (path && path.includes('/'))
    return (
      <Link to={path}>
        {icon && (React.isValidElement(icon) ? <span className="ant-menu-item-icon">{icon}</span> : <icon className="ant-menu-item-icon" />)}
        <span data-testid={messageId.toLowerCase + '-nav'}>{messages[messageId]}</span>
      </Link>
    );
  else {
    return (
...
const renderMenuItem = (item, sidebarColorSet, isSidebarBgImage, index) => {
  return item.type === 'collapse' ? (
    <Menu.SubMenu style={getStyles(item, sidebarColorSet, isSidebarBgImage, index, true)} key={item.path ? item.path : item.id} title={renderMenuItemChildren(item, sidebarColorSet, isSidebarBgImage)}>
      {item.children.map((item) => renderMenuItem(item, sidebarColorSet, isSidebarBgImage, index + 1))}
    </Menu.SubMenu>
  ) : (
    <Menu.Item key={item.id} style={getStyles(item, sidebarColorSet, isSidebarBgImage, index)}>
...
const renderMenu = (item, sidebarColorSet, isSidebarBgImage, index) => {
  return item.type === 'group' ? (
    <Menu.ItemGroup style={getStyles(item, sidebarColorSet, isSidebarBgImage, index, true)} key={item.path ? item.path : item.id} title={renderMenuItemChildren(item, sidebarColorSet, isSidebarBgImage)}>
      {item.children.map((item) => renderMenuItem(item, sidebarColorSet, isSidebarBgImage, index + 1))}
    </Menu.ItemGroup>
  ) : (
    <Menu.Item key={item.id} exact={item.exact} style={getStyles(item, sidebarColorSet, isSidebarBgImage, index)}>
      {item.children ? item.children : renderMenuItemChildren(item, sidebarColorSet, isSidebarBgImage, index)}
    </Menu.Item>
  );
};
