...
const renderMenuItem = (item, sidebarColorSet, index) => {
  return item.type === 'collapse' ? (
    <Menu.SubMenu style={getStyles(item, sidebarColorSet, index, true)} key={item.path ? item.path : item.id} title={renderMenuItemChildren(item, sidebarColorSet)}>
    ...
    <Menu.Item key={item.id} style={getStyles(item, sidebarColorSet, index)}>
      {item.children ? item.children : renderMenuItemChildren(item, sidebarColorSet)}
    </Menu.Item>
  );
};
...
const renderHorMenu = (item, sidebarColorSet, index) => {
  return item.type === 'group' ? (
    ...
    <Menu.Item key={item.id} exact={item.exact} style={getStyles(item, sidebarColorSet, index, true)}>
      {item.children ? item.children : renderMenuItemChildren(item, sidebarColorSet)}
    </Menu.Item>
  );
};
