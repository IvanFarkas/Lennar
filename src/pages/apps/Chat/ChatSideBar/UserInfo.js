  ...
  return (
    <div className="chat-user-info">
      <div className="chat-user-avatar-view">
        {user.photoURL || user.image ? <Avatar className="chat-user-avatar" src={user.photoURL || user.image} /> : <Avatar className="chat-user-avatar">{getUserAvatar()}</Avatar>}
        {showStatus && (
          <span
      ...
      <div className="chat-user-info-content">
        <h3 className="chat-user-name text-truncate">{user.displayName || user.name ? user.displayName || user.name : user.email}</h3>
        <span className="chat-user-status-text text-truncate">{user.status ? user.status : 'Online'}</span>
      </div>
    </div>
  );
};
