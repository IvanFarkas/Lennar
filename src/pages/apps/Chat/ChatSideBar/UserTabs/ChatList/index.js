...
const ChatList = ({ chatListData, loading }) => {
  ...
  return (
    <AppScrollbar className="chat-sidebar-scrollbar">
      ...
      <AppList data={chatListData} ListEmptyComponent={<ListEmptyResult content={messages['chatApp.noUserFound']} loading={loading} placeholder={<ChatListSkeleton />} />} renderItem={(item) => <ChatItem key={item.id} item={item} selectedUser={selectedUser} />} />
    </AppScrollbar>
  );
};
