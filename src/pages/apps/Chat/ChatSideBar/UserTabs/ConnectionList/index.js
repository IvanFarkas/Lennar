...
const ConnectionList = ({ connectionListData, loading }) => {
  ...
  return (
    <AppScrollbar className="chat-sidebar-scrollbar">
      ...
      <AppList data={connectionListData} ListEmptyComponent={<ListEmptyResult content={messages['chatApp.noUserFound']} loading={loading} placeholder={<ChatListSkeleton />} />} renderItem={(item) => <ConnectionItem key={item.id} item={item} selectedUser={selectedUser} />} />
    </AppScrollbar>
  );
};
