...
const CardComments = (props) => {
  ...
  const getCommentCell = (item, index, isPreviousSender) => {
    return (
      <div
        ...
        key={index}
      >
        {item.sender.image ? <Avatar src={item.sender.image} className="scrum-board-card-comment-item-user-avatar" /> : <Avatar className="scrum-board-card-comment-item-user-avatar">{item.sender.name.charAt(0).toUpperCase()}</Avatar>}
        <div className="scrum-board-card-comment-item-user-content">
          <span className="scrum-board-card-comment-item-user-date">{item.date}</span>
  ...
  return (
    <div className="scrum-board-card-comment-view">
      ...
      <AppScrollbar className="scrum-board-card-comment-scroll">{comments && comments.length > 0 ? <div className="scrum-board-card-comment">{comments.map((item, index) => getCommentCell(item, index, index > 0 && comments[index - 1].sender.id === item.sender.id))}</div> : null}</AppScrollbar>
