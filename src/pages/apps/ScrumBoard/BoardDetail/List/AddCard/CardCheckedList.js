...
const CardCheckedList = (props) => {
  const { onAddNewCheckedItem, checkedList, onSetCheckedItemText, onDeleteCheckedItem } = props;
  ...
  return (
    <div className="scrum-board-card-check-list">
      ...
      {checkedList.map((checkedItem) => {
        return (
          <div className="scrum-board-card-check-list-item" key={checkedItem.id}>
            <Checkbox className="scrum-board-card-check-list-item-check-box" />
            <Input className="scrum-board-card-check-list-item-input" value={checkedItem.title} onChange={(e) => onSetCheckedItemText(e.target.value, checkedItem.id)} />
            <AppIconButton icon={<AiOutlineDelete />} onClick={() => onDeleteCheckedItem(checkedItem.id)} />
          </div>
        );
      })}
    </div>
