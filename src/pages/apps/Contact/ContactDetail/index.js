...
const ContactDetail = (props) => {
  const { isShowDetail, selectedContact, onShowDetail, onSelectContactsForDelete, onOpenEditContact } = props;
  ...
  return (
    <>
      <Modal visible={isShowDetail} onOk={isShowDetail} footer={false} onCancel={() => onShowDetail(false)} className="contact-detail-modal">
        <div className="contact-detail-modal-header">
          <ContactActions onChangeStarred={onChangeStarred} onDeleteContact={onDeleteContact} onOpenEditContact={onOpenEditContact} contact={contact} />
          <div className="contact-detail-modal-user">
            {contact.image ? <Avatar className="contact-detail-modal-user-avatar" src={contact.image} /> : <Avatar className="contact-detail-modal-user-avatar">{contact.name[0].toUpperCase()}</Avatar>}
            <h4>{contact.name}</h4>
          </div>
        </div>
        ...
        <AppScrollbar className="contact-detail-modal-scrollbar">
          ...
          <div className="contact-form-footer">
            <Button type="primary" ghost className="contact-form-btn" onClick={onContactDetailClose}>
