  ...
  return (
    <>
      <div className="contact-sidebar-header">
        <Button ghost type="primary" icon={<PlusOutlined style={{ marginRight: 8 }} />} onClick={handleAddContactOpen}>
      ...
      <AppScrollbar className="contact-sidebar-scrollbar">
        <div className="contact-sidebar-content">
          <div className="contact-sidebar-list">
            <AppList animation={AppAnimates.SLIDELEFTIN} data={folderList} ListEmptyComponent={<ListEmptyResult loading={true} placeholder={<SidebarPlaceholder />} />} renderItem={(item) => <AppsSideBarFolderItem key={item.id} item={item} path={`/apps/contact/folder/${item.alias}`} />} />
          </div>
          ...
          <AppList animation={AppAnimates.SLIDELEFTIN} data={labelList} ListEmptyComponent={<ListEmptyResult loading={true} placeholder={<SidebarPlaceholder />} />} renderItem={(label) => <LabelItem key={label.id} label={label} />} />

          {isAddContact ? <CreateContact isAddContact={isAddContact} handleAddContactClose={handleAddContactClose} /> : null}
        </div>
      </AppScrollbar>
    </>
  );
