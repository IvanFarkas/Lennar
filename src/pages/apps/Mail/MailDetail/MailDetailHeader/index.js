...
import { onUpdateMailFolders, onUpdateSelectedMail } from '../../../../../redux/actions/MailApp';
...
const MailDetailHeader = ({ selectedMail }) => {
  ...
  const onSelectLabel = ({ key }) => {
    ...
    mail.label = labelList.find((label) => label.id.toString() === key.toString());
  ...
  const menuMoveTo = (
    <Menu>
      ...
      <Menu.Item onClick={onChangeStarredStatus}>{selectedMail.isStarred ? <IntlMessages id="mailApp.markAsNotImportant" /> : <IntlMessages id="mailApp.markAsImportant" />}</Menu.Item>
    </Menu>
  );
  ...
  return (
    <>
      <AppIconButton className="mail-detail-arrow" title={<IntlMessages id="common.back" />} icon={<BiArrowBack />} onClick={onClickBackButton} />
      ...
      <div className="mail-detail-header-action">
        ...
        <AppIconButton title={<IntlMessages id="mailApp.markAsUnread" />} icon={<HiOutlineMailOpen />} onClick={() => onChangeReadStatus()} />
