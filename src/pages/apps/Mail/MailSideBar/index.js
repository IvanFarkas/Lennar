      ...
      {labelList || connectionList || folderList ? (
        <>
          <div className="mail-sidebar-header">
            <Button ghost type="primary" icon={<PlusOutlined style={{ marginRight: 8 }} />} onClick={onOpenComposeMail}>
          ...
          <AppScrollbar className="mail-sidebar-scrollbar">
            <div className="mail-sidebar-content">
              <List className="mail-sidebar-list" component="nav" aria-label="main mailbox folders">
                <AppList data={folderList} ListEmptyComponent={<ListEmptyResult loading={true} placeholder={<SidebarPlaceholder />} />} renderItem={(item) => <AppsSideBarFolderItem key={item.id} item={item} path={`/apps/mail/folder/${item.alias}`} />} />
              </List>
              ...
              <List className="mail-sidebar-list" component="nav" aria-label="main mailbox folders">
                <AppList data={labelList} ListEmptyComponent={<ListEmptyResult loading={true} placeholder={<SidebarPlaceholder />} />} renderItem={(label) => <LabelItem key={label.id} label={label} />} />
              </List>
              <h5 className="mail-sidebar-title">
