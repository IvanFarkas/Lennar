...
const MailListItemMobile = (props) => {
  ...
  return (
    ...
    >
      ...
      >
        {checkedMails.includes(mail.id) ? <CheckOutlined /> : <Avatar className="mail-list-avatar-mobile">{getSenderName().charAt(0).toUpperCase()}</Avatar>}
      </div>

      <div className="mail-list-content-mobile">
        <div className="mail-list-main-content-mobile">
          <p className="mail-list-title-mobile text-truncate">{getSenderName()}</p>
          <p className="mail-list-sub-title-mobile text-truncate">{mail.subject}</p>
          <p className="mail-description-mobile text-truncate">{getStringFromHtml(getDescription())}</p>
        </div>
        ...
        <div className="mail-list-action-mobile">
          ...
          <span className="mail-list-starred-mobile" onClick={(event) => event.stopPropagation()}>
