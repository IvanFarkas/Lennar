...
const CheckedMailActions = (props) => {
  ...
  const onSelectLabel = ({ key }) => {
    const labelType = labelList.find((label) => label.id.toString() === key.toString());
  ...
  return (
    <div className="mail-checked-action">
      <AppIconButton title={<IntlMessages id="common.archive" />} onClick={() => onChangeMailFolder(127)} icon={<BiArchiveIn />} />

      <AppIconButton title={<IntlMessages id="common.reportSpam" />} onClick={() => onChangeMailFolder(125)} icon={<AiOutlineInfoCircle />} />

      <AppIconButton title={<IntlMessages id="common.trash" />} onClick={() => onChangeMailFolder(126)} icon={<AiOutlineDelete />} />
      ...
      <Dropdown overlay={menuMoveTo} trigger={['click']}>
        <AppIconButton title={<IntlMessages id="common.moveTo" />} icon={<HiOutlineFolderRemove />} />
      </Dropdown>
    </div>
  );
};
