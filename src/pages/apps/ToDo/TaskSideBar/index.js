  ...
  return (
    <>
      <div className="todo-sidebar-header">
        <Button ghost type="primary" icon={<PlusOutlined style={{ marginRight: 8 }} />} onClick={onOpenAddTask}>
      ...
      <AppScrollbar className="todo-sidebar-scrollbar">
        <div className="todo-sidebar-content">
          <List className="todo-sidebar-list" itemLayout="horizontal">
            <AppList data={folderList} ListEmptyComponent={<ListEmptyResult loading={true} placeholder={<SidebarPlaceholder />} />} renderItem={(item) => <AppsSideBarFolderItem key={item.id} item={item} path={`/apps/todo/folder/${item.alias}`} />} />
          </List>
