...
const TaskLabel = ({ selectedTask }) => {
  ...
  return (
    <Select placeholder={messages['common.label']} maxTagCount={2} style={{ minWidth: 100 }} mode="multiple" defaultValue={selectedTask.label.map((label) => label.id)} onChange={onChangePriority}>
      {labelList.map((label) => {
        return (
