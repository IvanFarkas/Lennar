...
const CommentsListItem = ({ item, isPreviousSender, isLast }) => {
  return (
    <div className={clsx('todo-comment-item', isPreviousSender ? 'hideUserInfo' : 'first-chat-message', isLast ? 'last-chat-message' : '')}>
      <div className="todo-comment-view">
        <div className="todo-comment-user">
          {item.image ? <Avatar src={item.image} className="todo-avatar-36" /> : <Avatar className="todo-avatar-36 todo-avatar-name">{item.name[0]}</Avatar>}
