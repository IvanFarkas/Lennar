...
const ChangeStaff = ({ handleStaffChange, selectedStaff }) => {
  ...
  return (
    <Form.Item className="form-field">
      <Select defaultValue={selectedStaff?.id} placeholder={messages['common.staff']} onSelect={handleStaffChange} style={{ minWidth: 150 }}>
        {staffList.map((staff) => {
          return (
            <Option value={staff.id} key={staff.id}>
              <div className="ant-row ant-row-middle">
                {staff.image ? <Avatar className="todo-ch-staff-avatar" src={staff.image} /> : <Avatar className="todo-ch-staff-avatar">{staff.name.toUpperCase()}</Avatar>}
