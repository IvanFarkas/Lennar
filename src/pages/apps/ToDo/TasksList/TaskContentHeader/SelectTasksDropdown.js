...
const SelectTasksDropdown = ({ onSelectTasks }) => {
  ...
  return (
    <span className="todo-select-task-dropdown-view">
      <Select value={selectedItems} onChange={onChangeSelectValue} defaultValue={messages['common.all']} className="todo-select-task-dropdown">
        ...
        <Select.Option value={3}>{messages['common.attachments']}</Select.Option>
      </Select>
    </span>
  );
};
