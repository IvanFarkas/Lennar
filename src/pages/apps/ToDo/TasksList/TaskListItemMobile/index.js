...
const TaskListItemMobile = ({ task, checkedTasks, onChangeCheckedTasks, match, onChangeStarred }) => {
  ...
  return (
    ...
    >
      <div className="todo-list-mobile-main-content">
        ...
        >
          {checkedTasks.includes(task.id) ? <CheckOutlined /> : <Avatar className="todo-list-mobile-avatar" src={task?.assignedTo?.image} />}
        </div>
      ...
      <div className="todo-list-mobile-action">
        <span className="todo-list-mobile-date">{moment(task.startDate).format('HH:mm A')}</span>
        <span className="todo-list-star-mobile" onClick={(event) => event.stopPropagation()}>
