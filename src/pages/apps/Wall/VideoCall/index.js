...
const VideoCall = ({ data }) => {
  ...
  return (
    <AppCard className="video-call-card" cover={<img src={coverImg} alt="V-card" />}>
      ...
      <div className="video-call">
        ...
        <div className="video-call-avatar-view">
          {users.slice(0, 4).map((user) => (
            <Avatar key={user.id} className="video-call-avatar" src={user.profilePic} />
          ))}
          {users.length > 4 ? <span className="video-call-avatar-count">+{users.length - 4}</span> : null}
        </div>
