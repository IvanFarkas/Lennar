...
const KnowledgeBase = () => {
  ...
  const installationQueries = filterText !== '' ? installationData.filter((data) => data.ques.includes(filterText)) : installationData;
    ...
    <div className="know-base">
      ...
      <AppAnimateGroup type="bottom" animateStyle={{ height: 'auto' }}>
        <div className="know-base-header" key="a">
          ...
          <Search className="know-search" placeholder={messages['knowledge.AppSkeleton']} onSearch={onSearch} value={filterText} onChange={(event) => setFilterText(event.target.value)} />
        </div>
      </AppAnimateGroup>
