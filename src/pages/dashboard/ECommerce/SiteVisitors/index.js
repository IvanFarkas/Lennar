...
const SiteVisitors = ({ siteVisitorsData }) => {
  ...
  return (
    <AppCard heightFull className="no-card-space-ltr-rtl site-visitor-card" title={messages['eCommerce.siteVisitorsStatistics']}>
      <AppRowContainer>
        <Col xs={24} md={7} xl={7} xxl={6}>
          <h3 className="site-visitor-title">{messages['eCommerce.countries']}</h3>
          <List dataSource={siteVisitorsData} renderItem={(data) => <CountryCell key={data.id} data={data} />} />
        </Col>
        ...
        <Col xs={24} md={7} xl={7} xxl={6}>
          <h3 className="site-visitor-title">{messages['eCommerce.countries']}</h3>
          <List dataSource={siteVisitorsData} renderItem={(data) => <CountryCell key={'sec-c-' + data.id} data={data} />} />
        </Col>
      </AppRowContainer>
    </AppCard>
  );
