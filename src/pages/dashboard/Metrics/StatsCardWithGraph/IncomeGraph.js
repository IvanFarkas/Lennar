  ...
  return (
    <ResponsiveContainer height={200} width="100%">
      <AreaChart data={data}>
        ...
        <Area type="monotone" dataKey="revenue" stroke="#FFA940" strokeWidth={3} fill="url(#color15)" dot={{ r: 0 }} />
        <Tooltip
          labelStyle={{ color: 'black' }}
          cursor={false}
          content={(data) => {
            return data.payload[0] ? <span className="stats-income-graph-para">{data.payload[0].payload.revenue}</span> : null;
          }}
          wrapperStyle={{
            background: '#FFA940',
            borderRadius: 10,
