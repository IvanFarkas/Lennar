  ...
  return (
    <AppCard heightFull className="video-promo-card">
      <div className="react-player-view">
        <ReactPlayer className="react-player" controls={true} url="https://www.youtube.com/embed/X1dz0xRbSJc" />
      </div>
      <div className="video-promo">
        ...
        <Tabs className="video-promo-tabs" defaultActiveKey="1" onChange={callback}>
          {tabs.map((item, index) => (
          ...
          <div className="video-tabs-content" key={index}>
            ...
            <div className="video-tabs-action">
              <div className="video-tabs-action-left">
                <Button size="small" className="btn-primary-outline" style={{ fontSize: 10, whiteSpace: 'nowrap' }}>
                  See Calendar
                </Button>
                <Button size="small" style={{ fontSize: 10, whiteSpace: 'nowrap' }} type="primary">
                  View details
                </Button>
              </div>
              <div className="video-tabs-action-right">
                <span>{item.students} Students enrolled</span>
                <span className="text-secondary">{item.daysLeft} Days left</span>
              </div>
            </div>
          </div>
        ))}
