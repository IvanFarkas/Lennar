    ...
    <List.Item key={item.id} className="my-class-item item-hover">
      ...
      <div className="my-class-content">
        ...
        <div className="my-class-progress-view">
          <Progress percent={item.percent} strokeColor="#0A8FDC" trailColor="#d6d6d6" strokeWidth={5} showInfo={false} />
        </div>
      </div>
    </List.Item>
  );
...
const MyClass = ({ classData }) => {
  ...
  return (
    <AppCard heightFull className="no-card-space-ltr-rtl" title={messages['academy.myClass']}>
      <List dataSource={classData} renderItem={(data, index) => <ClassItem key={index} item={data} />} />
    </AppCard>
  );
};
