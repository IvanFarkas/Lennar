...
const SalesState = ({ salesState, saleChartData }) => {
  ...
  return (
    <AppCard className="sales-state-card" title={messages['dashboard.analytics.salesState']} heightFull extra={<AppSelect menus={[messages['dashboard.thisWeek'], messages['dashboard.lastWeeks'], messages['dashboard.lastMonth']]} defaultValue={messages['dashboard.thisWeek']} onChange={handleSelectionType} />}>
      <p className="sales-state-sub-title">1343 {messages['dashboard.analytics.salesThisWeek']}</p>
          ...
          <Col xs={24} sm={10} className="mb-0">
            <div className="sales-state-content">
              <AppList
                data={salesState}
                renderItem={(item) => (
                  <div className="sales-state-item" key={'salesState-' + item.id}>
